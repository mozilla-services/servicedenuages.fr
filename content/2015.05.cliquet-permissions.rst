...
- Les applications qui intéragissent en leur nom.
...
puisque nous n'avons pas trouvé de cas d'utilisation qui les necessitent.
...
|                +------------------------+-----------------------------------+
|                | `groups:create`        | Permission de créer des gorupes   |
|                |                        | dans le bucket.                   |
+----------------+------------------------+-----------------------------------+
...
wikis sont privés par défault et certaines pages peuvent être rendues publiques:
...
Pour le ``PATCH`` nous utilisons la syntaxe prefixée par un ``+`` ou
par un ``-`` pour ajouter ou enlever des principals sur un ACL.

Il est également possible de faire un PUT pour réinialiser les ACLs,
...
personne n'à accès aux données.
...
Voilà ou nous en sommes de notre reflexion !

Si vous avez des choses à rajouter, des points de désaccord ou autres
reflexions, n'hésitez pas à nous interrompre pendant qu'il en est encore temps
!
