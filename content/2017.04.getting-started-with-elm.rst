...
language, a compiler, an architecture and tooling around. It is focused
...
making sure you app works on all the available browser versions.
...
You can think of the Elm platform as the equivalent feature set of React_,
Redux_, react-redux_, Babel_, eslint_, Flow/Typescript and Webpack_ all
bundled in a single homogeneous package!
...
code will be transpiled to JavaScrit by the compiler.
...
browser. Also that means less unit tests and defensive priogramming to
...
run ``elm-format`` each time you save your file which is really handy.
