...
![Couché de soleil sur Douarnenez (@phrawzty)]({filename}/images/notifications-douarnenez.jpg)

# tl;dr

Pour ceux qui sont pressé, voici un petit résumé : Nous allons ajouter
un système de "notifications" à Kinto, sans perdre de vue la
simplicité, la modernité et la flexibilité. Notre objectif est de
...
Si ça vous souhaitez en savoir plus, lisez la suite.

# Pourquoi ?

La réponse la plus simple est que nous en avons besoin à Mozilla. Nous
...
nous avons besoin de stocker et de transmettre de petits blocks d'informations à
la fois entre des systèmes internes mais aussi avec l'ensemble de nos
utilisateurs.

Les fonctionnalités de Kinto répondent parfaitement au besoin si ce
n'est pour notifier les utilisateurs lors des modifications.

Aucun problème. Il ne reste plus qu'à ajouter la fonctionnalité.

## Mais attention, à ne pas oublier une petite chose !

Bien sur, Kinto est un projet Mozilla - mais il a été conçu comme une
solution générique, ce qui veut dire que tous ce que nous ajoutons
doit rester raisonablement générique.
...
avant tout orienté vers le web.

En faisant le tour des paramètrages qu'une solution raisonablement
générique devrait proposer, nous avons défini les exigences suivantes
pour notre système de notifications :

* S'efforcer d'être agnostic autant que possible, c'est à dire ne pas
  obliger les développeurs à utiliser telle ou telle solution,
  service, backend voire même tel language de programmation.
* Puisque nous sommes orienté vers le web, utiliser le protocole HTTP dès que possible.
* Le système de notification doit être capable de passer à l'échelle
  facilement ; cela s'applique à tous les éventuelles systèmes de
  queuing, workers, etc.
* Pour finir ce doit être un système robuste, nous ne souhaitons pas perdre de messages.

# Qu'est-ce que nous essayons de faire ?

Avant de descendre dans les menus détails, prenons un moment pour
...
* Un *message* est un morceau d'information qui est envoyé d'un
  ...
  ou être généré en interne.
* Une *action* est un processus qui est enclenché en fonction du
  ...
  une fonctionalités de Kinto, un programme local, une fonctionalité
  ajoutée par un plugin ou tout autre chose.
* Une *resultat*, comme son nom l'indique, est ce qui résulte de
  ...
  un packet JSON. Mais ça peut également être le résultat de la suite
  de tests (les possibilités sont sans limites).
* Pris dans leur ensemble, les *évènements* peuvent être vu comme un
...
> Une notification est donc la combinaison d'un message et d'une
  action déclenché suite à un évènement. Le résultat, bien que
...
qu'il sera traité. Cela permet de bien découplé les différents
éléments du système.
...
queue (dans Redis ou RabbitMQ par exemple) et c'est tout.
...
* Ce modèle impact peu Kinto, Kinto passe simplement le message à la
...
* En fonction de si la communauté Kinto est active il serait
  rapidement possible d'avoir le support d'un nombre raisonnable de
  service externes de queuing.

Inconvénients:

* Chaque service de queue nécessirait un module spécifique à coder et
  qui devrait être maintenu par la communauté.  * L'utilisateur final
  (destinataire, destination ou autre) du message serait responsable
  de l'intégration avec le protocole du système de queue.
  Autrement dit, si Kinto est configuré pour utiliser RabbitMQ, il
  faudra que le consommateur des messages parle RabbitMQ également.
...
puisqu'il impose une technologie au consomateur des messages.

C'est un modèle un peu dangereux car il n'y a aucune garantie que le
message arrivent à son destinataire (bien que tous les systèmes de
...
Kinto et de proposer une interface sur laquelle le consomateur puisse
s'enregistrer.
...
Ce modèle à certains avantages ; cependant, la quantité de code
...
système de queues sans respecter notre règle d'agnostisme technique.
Enfin, le passage à l'échelle n'apparait pas extrêmement flexible
(bien que ça reste de la théorie) - ça semble dommage d'augmenter deux
...
comme le fond les jolies stacks basées sur des micro-services.

Cela permet de découpler le système de queues de Kinto tout en
garantissant l'interroperabilité et le contrôle.
...
* Permet un ensemble de solution très flexibles qui permette de
  connecter à peu près tout ce qu'on veut.
...
métiers alors que ce serait innaproprié de les implémenter dans Kinto.
...
* Définir les types d'évènements, s'il faut que l'évènement soit
  déclenché avant ou après ainsi que la structure des informations
  envoyés lors d'un évènement.
* Activer des évènements Python synchrones en utilisant [le système
  d'évènements de
...
* Implémenter un backend de notifications dans Kinto permettant de
  s'enregister sur ces évènements.
...
Nous aimons bien l'idée d'avoir des consomateurs qui viendrait
...
Nous écrivons avant tous ces articles pour échanger avec vous, alors
n'hésitez surtout pas à poster vos questions, remarques,
intérrogations via les commentaires par exemple.
