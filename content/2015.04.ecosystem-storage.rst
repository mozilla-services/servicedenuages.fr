...
we want to build a solution for generic data storage. We're rebooting `Daybed <http://daybed.readthedocs.io>`_
at Mozilla!
...
`Cliquet <http://cliquet.readthedocs.io/>`_. We had a chance to setup `a very simple protocol <http://cliquet.readthedocs.io/en/latest/api/>`_, strongly inspired by
...
<http://cliquet.readthedocs.io/en/latest/rationale.html>`_, will of course be honoured :)
...
<http://kinto.readthedocs.io>`_, all the automation work of deploying
